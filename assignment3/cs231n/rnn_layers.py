...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for i in range(T):
        next_h, cache_next = rnn_step_forward(x[:, i, :], h0, Wx, Wh, b)
        h[:, i, :] = next_h
        h0 = next_h
...
def rnn_backward(dh, cache):
    ...
    for i in range(T):
        idx = T - i - 1        
        dxt, dht, dWxt, dWht, dbt = rnn_step_backward(dh[:, idx, :] + dprev_h, cache[idx])
        dx[:, idx, :] += dxt
        dprev_h = dht
        dh0 = dht
        dWx += dWxt
...
def word_embedding_backward(dout, cache):
    ...
    #for i in range(N):
        #for j in range(T):
            #dW[x[i, j], :] += dout[i, j, :] # this works too
    np.add.at(dW, x, dout)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    prev_c = np.zeros((N, H))
    for i in range(T):
        next_h, next_c, cache_next = lstm_step_forward(x[:, i, :], h0, prev_c, Wx, Wh, b)
        h[:, i, :] = next_h
        h0 = next_h
        prev_c = next_c
        cache.append(cache_next)
...
def lstm_backward(dh, cache):
    ...
    for i in range(T):
        idx = T - i - 1        
        dxt, dht, dct, dWxt, dWht, dbt = lstm_step_backward(dh[:, idx, :] + dprev_h, dprev_c, cache[idx])
        dx[:, idx, :] += dxt
        dprev_h = dht
        dprev_c = dct
        dh0 = dht
